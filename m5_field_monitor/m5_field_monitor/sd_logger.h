...
// Separate text-log file (e.g. startup banner, detection events, a
// periodic heartbeat) -- kept entirely independent of the binary MP3
// write path above (own File handle, own buffer-free line-at-a-time
// writes with an immediate flush per line, since log lines are rare and
// small compared to MP3's continuous high-rate byte stream, so there's
// no throughput reason to batch them the way MP3 writes are batched).
// Exists so a field-deployed device's results are recoverable from the
// SD card alone -- Serial output isn't recoverable once nothing's
// listening to it.
bool sdLoggerOpenLog(const char *filename);
void sdLoggerLogLine(const char *line);
void sdLoggerCloseLog(void);
